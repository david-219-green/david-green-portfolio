...
# Usage: .\scripts\extract-frames.ps1 -Clip 1 [-Frames 100] [-Width 1600] [-BudgetMB 6]
...
$src = Join-Path $root "generation\clip-$Clip.mp4"
...
foreach ($q in 72, 62, 52, 42, 34) {
    if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force -Confirm:$false }
    New-Item -ItemType Directory -Force $outDir | Out-Null
    & $ff -y -loglevel error -i $src -vf "fps=$fps,scale=${Width}:-2" -frames:v $Frames -c:v libwebp -q:v $q (Join-Path $outDir 'frame-%03d.webp')
    $files = Get-ChildItem $outDir -Filter *.webp
    $mb = [Math]::Round(($files | Measure-Object Length -Sum).Sum / 1MB, 2)
    if ($mb -le $BudgetMB) {
        "clip-{0}: {1} frames @ q={2} -> {3} MB" -f $Clip, $files.Count, $q, $mb
        return
    }
}
